...
<style>
    ...
    .role-badge {
        display: inline-block; background: rgba(201,162,75,0.16); color: #e8cf8c;
...
def render_sidebar_nav():
    with st.sidebar:
        ...
        pages = ["Dashboard", "Recommendation", "Top 10 Rankings"]
...
def render_dashboard():
    ...
    st.write("Use the navigation on the left to run agent recommendations, view Top 10 rankings, "
    ...
    with qa1:
        if st.button("🎯 Go to Recommendation", use_container_width=True):
            ...
            st.rerun()
    with qa2:
        if st.button("📋 Go to Top 10 Rankings", use_container_width=True):
            st.session_state.nav_page = "Top 10 Rankings"
            st.rerun()
...
def render_recommendation_page():
    ...
    # ---- SCREEN 7 — TOP 5 AGENTS (existing — untouched) ----
...
# ==============================================================================
# NEW FUNCTIONALITY — TOP 10 RANKINGS PAGE
# (Reuses the existing eligible_pool + compute_scores functions and the same
#  tie-break rule as the existing Top 5. Only the display slice — 10 instead
#  of 5 — and the extra columns are new.)
# ==============================================================================
def render_top10_page():
    render_header()
    st.markdown('<div class="section-title">📋 Top 10 Agent Rankings</div>', unsafe_allow_html=True)

    df = get_active_dataset()
    if df is None:
        st.info("⬆️ No dataset loaded yet. Go to **Recommendation** to upload an Excel file"
                + (", or use **Admin Console → Data Management**." if st.session_state.role == "Admin" else "."))
        return

    categories = sorted(df["Call Category"].dropna().unique().tolist())
    selected_category = st.selectbox("Select Call Category", categories, key="top10_category")
    ...
    pool = eligible_pool(cat_df)                      # existing eligibility logic
    scored = compute_scores(pool)                      # existing scoring logic
    scored = scored.sort_values(["Performance Score", "Average AHT (sec)"], ascending=[False, True])

    top10 = scored.head(10).reset_index(drop=True)     # NEW: 10 instead of 5
    top10.insert(0, "Rank", range(1, len(top10) + 1))
    top10["Call Category"] = selected_category

    display_cols = ["Rank", "Agent ID", "Agent Name", "Call Category", "Average AHT (sec)", "Performance Score"]
    for opt_col in ["Quality Score (%)", "FCR (%)", "Transfer Rate (%)", "Availability (%)",
                    "Calls Handled", "Experience (Months)"]:
    ...
    if len(top10) < 10:
        ...
        st.caption(f"Showing top 10 of {len(scored)} eligible agents for {selected_category}, "
                   "ranked by the platform's Performance Score.")

    with st.expander("ℹ️ How ranking works"):
        st.markdown(
            "Uses the exact same eligibility filter and weighted Performance Score as the "
            "Recommendation page — this view simply extends the result list from 5 to 10 agents, "
            "with a few additional reference columns (Call Category, Calls Handled, Experience)."
        )
...
def render_admin_console():
    ...
    with tab_data:
        ...
        st.caption("Uploading a new file here replaces the dataset used across the whole platform "
                   "(Recommendation and Top 10 Rankings pages).")
